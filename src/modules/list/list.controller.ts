...
import { GetListAccessGuard, ListGuard, RestoreDeleteListGuard, UpdateArchiveListGuard } from "src/utils/guards/list/list.guard";
@ApiTags('Lists')
@Controller('lists')
export class ListController {
    ...
    @Put('update-list')
    @ApiOperation({ summary: 'Update a list (by owner or by team member)'})
    @UseGuards(AuthGuard, UpdateArchiveListGuard)
    async updateList(@Body() body: UpdateListDto){
        return await this.listService.updateList(body);
    }
    @Delete('archive-list')
    @ApiOperation({summary: 'Archiving list (by owner or by team member)'})
    @UseGuards(AuthGuard, UpdateArchiveListGuard)
    ...
    @Put('restore-list/:id')
    @ApiOperation({summary: 'Restoring list (by owner or by team member)'})
    @UseGuards(AuthGuard, RestoreDeleteListGuard)
    async restoreList(@Param('id') id: string){
        return await this.listService.restoreList(id);
    }
    @Delete('delete-list/:id')
    @ApiOperation({summary: 'Deleting list (by owner or by team member)'})
    @UseGuards(AuthGuard, RestoreDeleteListGuard)
